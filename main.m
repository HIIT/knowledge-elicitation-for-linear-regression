...
num_nonzero_features = 5; % features that are nonzero
num_traingdata = 10; % number of samples (patients with available drug response)
...
theta_star = rand( num_nonzero_features, 1);
...
X = [X_all(1:num_traingdata,:)]'; % a subset of data that with available drug response
