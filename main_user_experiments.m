...
Loss_3 = zeros(num_methods, num_iterations, num_runs, num_users);
    ...
    for user = 1:num_users
        ...
        for method_num = 1:num_methods
            ...
            if find(strcmp(Method_list_GT, method_name))
                ...
                Loss_1(method_num, :, run, user) = mse;
                Loss_2(method_num, :, run, user) = log_pp_test;
                Loss_3(method_num, :, run, user) = log_pp_train;
                continue
            end
            ...
            for it = 1:num_iterations %number of user feedback
                ...
                [mse,log_pp_test,log_pp_train] = calculate_loss(x_train, y_train, posterior, ...
                    x_test, Y_hat, Y_test, model_params);
                Loss_1(method_num, it, run, user) = mse;
                Loss_2(method_num, it, run, user) = log_pp_test;
                Loss_3(method_num, it, run, user) = log_pp_train;
